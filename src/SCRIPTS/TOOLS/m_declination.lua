...
local WMM_PATH = "/WMM.COF"
local WMM_SIZE_STANDARD = 12 
-- local WMM_SIZE_HIGH_RESOLUTION = 133
...
local function load_wmm_cof(path)
  ...
  if not f then return nil, "WMM file saknas: " .. path end
  for line in f:lines() do
    line = trim(line)
    if line == "" then goto continue end
    local epoch = line:match("^%s*%d%d%d%d%.%d")
    if epoch and not WMM.epoch then WMM.epoch = tonumber(epoch) end
    local n,m,gnm,hnm,dgnm,dhnm = line:match("^(%d+)%s+(%d+)%s+([%-%d%.]+)%s+([%-%d%.]+)%s+([%-%d%.]+)%s+([%-%d%.]+)")
    if n then
...
local function wmm_declination(lat_deg, lon_deg, alt_m, date_year)
  ...
  local dt = (date_year or (os.date("*t").year + os.date("*t").yday/365.25)) - (WMM.epoch or os.date("%Y"))
...
local function run(event)
  ...
  lcd.drawText(2,28, state.msg or "")
  ...
  if not state.wmm_ok then
    lcd.drawText(2,50,"Ingen WMM-fil. Placera " .. WMM_PATH)
  else
    if state.gps.lat and state.gps.lon then
      local now = os.date("*t")
      state.year_dec = now.year + now.yday/365.25
      local decl, incl = wmm_declination(state.gps.lat, state.gps.lon, state.gps.alt, state.year_dec)
      state.decl, state.incl = decl, incl

      lcd.drawText(2,50, string.format("Lat %.6f  Lon %.6f", state.gps.lat, state.gps.lon))
      lcd.drawText(2,66, string.format("Decl: %.3f°  Incl: %.3f°", round(decl,3), round(incl,3)))
      ...
      lcd.drawText(2,50,"Väntar på GPS-telemetri...")
      lcd.drawText(2,66,"Se till att FC skickar GPS via telemetry/MSP")
    end
  end
